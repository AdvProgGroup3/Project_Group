...
//		
		...
		checkIn=new CheckIn(checkInData);
 		...
 		boolean fillInOrNot=checkIn.getFillInOrNot();
// 		boolean checkInOrNot=checkIn.getCheckInOrNot();
// 		Cookie cookie=new Cookie("checkInOrNot",String.valueOf(checkInOrNot));
// 		response.addCookie(cookie);
 		System.out.println("F"+fillInOrNot);
 		System.out.println(checkIn.getWarn().size());
 		Object data;
 		if(fillInOrNot==true &&checkIn.getWarn().size()==0){
 			data="<form action=SpacesList method=GET >";
 		...
 		} else if(fillInOrNot==false) {
 			
 			data="<div class='list-group-item'>"
 					+ "<br/>"
 					+ "The place is being used"
 					+ "<br/>"
 					+ "<br/>"
 					+ "<form action='SpacesList' method='GET'>"
 					+ "<button>Back</button>" 					
 					+ "</form>"
 					+ "</div>";
 			checkIn.setStatus(true);
     		...
     		request.setAttribute("data", data);
 		    request.getRequestDispatcher("/Checkout.jsp").forward(request, response);
// 			response.sendRedirect("SpacesList");
 			
 		}else if(!areaId.equals(checkIn.getWarn().get("areaId"))){
 			...
 			data+="<div class='list-group-item'>"
 					...
 					+ "<br/>"
 					+ "<br/>"
 					+ "<button type='submit' name='areaId' value='"+checkIn.getWarn().get("areaId")+"'>Check Out</button>"
 					+ "</div>";
